...
def vllm_infer(
    ...
    **kwargs
    ...
    vllm_model = LLM(**engine_args)
    results = vllm_model.generate(inputs, sampling_params, lora_request=lora_request)
    # preds = [result.outputs[0].text for result in results]
    preds = [[output.text for output in result.outputs] for result in results]
    
    if get_device_count() == 0 or os.environ.get("LOCAL_RANK", "0") == "0":
        os.makedirs(os.path.dirname(save_name), exist_ok=True)
        with open(save_name, "w", encoding="utf-8") as f:
            for text, pred, label in zip(prompts, preds, labels):
                f.write(json.dumps({"predict": pred, "prompt": text, "label": label}, ensure_ascii=False) + "\n")
